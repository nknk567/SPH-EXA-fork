...
//! @brief count particles whose final h is pinned at the cumulative walls hWallFactor*h0 / 0.5*h0
template<class Dataset, class Tv>
extern std::pair<size_t, size_t> countHWallPinned(const GroupView&, Dataset& d, const Tv* h0, float hWallFactor);
...
template<class Dataset>
extern void computeIadDivvCurlvGradh(Dataset& d, const cstone::Box<typename Dataset::RealType>&, bool nrMode,
                                     float gradhMin);
